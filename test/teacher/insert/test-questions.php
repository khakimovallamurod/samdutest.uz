...
try {
    ...
    if (!column_exists($db, 'test_questions', 'question_type')) {
        $db->query("ALTER TABLE test_questions ADD COLUMN question_type ENUM('open','closed') NOT NULL DEFAULT 'closed'");
    }
    ...
    foreach ((array) $items as $it) {
        $q = $sanitize($it['question_text'] ?? '');
        if (trim(strip_tags($q)) === '') {
            continue;
        }
        $qt = trim((string) ($it['question_type'] ?? 'closed'));
        if (!in_array($qt, ['open', 'closed'], true)) {
            $qt = 'closed';
        }
        $a = $sanitize($it['option_a'] ?? '');
        $b = $sanitize($it['option_b'] ?? '');
        $c = $sanitize($it['option_c'] ?? '');
        $d = $sanitize($it['option_d'] ?? '');
        $co = trim((string) ($it['correct_option'] ?? ''));
        ...
        $st->bind_param('iisssssss', $testId, $teacherId, $q, $qt, $a, $b, $c, $d, $co);
        if ($st->execute()) {
            $created++;
        }
    }
    ...
    $st->close();
    flash_set($created > 0 ? 'success' : 'error', $created > 0 ? 'Savollar saqlandi.' : 'Savol topilmadi.');
} catch (Throwable $e) {
    error_log('Insert test questions failed: ' . $e->getMessage());
